...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        //Comparator<Record> recordComparer = new RecordComparator();
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        // TODO(proj3_part1): implement
        PriorityQueue<Pair<Record, Integer>> allRuns = new PriorityQueue<>(runs.size(), new RecordPairComparator());
        List<BacktrackingIterator<Record>> runIterators = new ArrayList<>();
        Run mergedRuns = makeRun();

        for (int i = 0; i < runs.size(); i++) {
            BacktrackingIterator<Record> runIter = runs.get(i).iterator();
            Record r = runIter.next();

            runIterators.add(i, runIter);
            allRuns.add(new Pair<Record,Integer>(r, i));
        }

        while (!allRuns.isEmpty()) {
            Pair<Record, Integer> temp = allRuns.poll();
            Record r = temp.getFirst();
            int i = temp.getSecond();
            BacktrackingIterator<Record> tempIter = runIterators.get(i);

            mergedRuns.add(r);

            if (tempIter.hasNext()) {
                allRuns.add(new Pair<Record,Integer>(tempIter.next(), i));
            }
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        List<Run> tempList = new ArrayList<>();
        int counter = 0;
        for (int i = 0; i < runs.size(); i++) {
            if (counter == (numBuffers - 1) || i == (runs.size() - 1)) {
                if (i == (runs.size() - 1)) {
                    tempList.add(runs.get(i));
                }
                mergedRunList.add(mergeSortedRuns(tempList));
                tempList.clear();
                counter = 0;
            } 
            if (counter < numBuffers - 1) {
                tempList.add(runs.get(i));
                counter++;
            } 
        }
    ...
    public Run sort() {
        ...
        while (sourceIterator.hasNext()) {
            BacktrackingIterator<Record> allRecordIter = QueryOperator.getBlockIterator(sourceIterator, this.computeSchema(), numBuffers);
            allRuns.add(sortRun(allRecordIter));
        }
        if (allRuns.size() == 1) {
            return allRuns.get(0);
        }
        while (allRuns.size() > 1) {
            allRuns = mergePass(allRuns);
        }
        return allRuns.get(0); 
    }
