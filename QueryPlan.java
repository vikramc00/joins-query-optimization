...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        List<Integer> availableIndices = getEligibleIndexColumns(table);
        ...
        for (int index : availableIndices) {
            ...
            SelectPredicate currPredicate = this.selectPredicates.get(index);
            QueryOperator currOperator = new IndexScanOperator(this.transaction, table, currPredicate.column, currPredicate.operator, currPredicate.value);
            int opIOCost = currOperator.estimateIOCost();
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        QueryOperator cheapestJoinOp;

        for (Set<String> currTableSet : prevMap.keySet()) {
            for (JoinPredicate currPredicate : this.joinPredicates) {

                String leftCol = currPredicate.leftColumn;
                String leftTable = currPredicate.leftTable;
                String rightCol = currPredicate.rightColumn;
                String rightTable = currPredicate.rightTable;
                HashSet<String> returnHash = new HashSet<String>();

                if (currTableSet.contains(leftTable) && !currTableSet.contains(rightTable)) {

                    QueryOperator tableOp = pass1Map.get(new HashSet<String>(Arrays.asList(rightTable)));
                    cheapestJoinOp = minCostJoinType(prevMap.get(currTableSet), tableOp, leftCol, rightCol);

                } else if (currTableSet.contains(rightTable) && !currTableSet.contains(leftTable)) {

                    QueryOperator tableOp = pass1Map.get(new HashSet<String>(Arrays.asList(leftTable)));
                    cheapestJoinOp = minCostJoinType(tableOp, prevMap.get(currTableSet), leftCol, rightCol);

                } else {
                    continue;
                }

                returnHash.addAll(currTableSet);
                returnHash.add(leftTable);
                returnHash.add(rightTable);
                
                result.put(returnHash, cheapestJoinOp);
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set<String>, QueryOperator> initialPass = new HashMap<>();
        initialPass.put(new HashSet<String>(Arrays.asList(this.tableNames.get(0))), minCostSingleAccess(this.tableNames.get(0)));

        if (tableNames.size() > 1) {
            for (String currTable : tableNames) {
                initialPass.put(new HashSet<String>(Arrays.asList(currTable)), minCostSingleAccess(currTable));
            }
        }

        Map<Set<String>, QueryOperator> minPass = new HashMap<>(initialPass);

        if (tableNames.size() > 1) {
            for (int i = 0; i < tableNames.size() - 1; i++){
                minPass = minCostJoins(minPass, initialPass);
            }  
        }

        this.finalOperator = minCostOperator(minPass);  
        addGroupBy();
        addProject();
